  ...
  <Row>
    <iCol span="24" class="top-30">
      <Card>
        ...
        <a href="#" slot="extra">
          <Button type="success" shape="circle" v-on:click="$router.push({path: 'infoAdd'});" icon="plus">添加</Button>
      ...
      return {
        ...
        columns7: [
          ...
          },
          {
            ...
            align: 'center',
            render: (h, params) => {
              return h('span', this.data[params.index].isFrontDisplay === 1 ? '在线' : '已下线');
            }
          },
          {
            title: '操作',
...
//                    icon: 'eye'
                  ...
                  on: {
                    click: () => {
                      this.$router.push({path: 'infoEdit', query: {'id': this.data[params.index].id}});
                    }
                  }
                }, '编辑'),
                h('Button', {
...
//                    icon: 'trash-a'
    ...
    methods: {
      init: async function () {
        await cmsPage({'currentPage': this.pageNumber, 'pageSize': this.pageSize, 'typeCode': '300'}).then(r => {
